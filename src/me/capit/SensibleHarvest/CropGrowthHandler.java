package me.capit.SensibleHarvest;
...
public class CropGrowthHandler implements Listener{
	...
	public void onTreeGrowth(StructureGrowEvent e){
		...
		Biome biome = bl.getBiome();
		double cfgchance = plugin.getConfig().getDouble("config.global.RATE");
		if ((chance <= cfgchance && (bl.getLightFromSky()>=4) || isUnderGreenhouse(bl, true))){
	...
	public boolean isUnderGreenhouse(Block b, boolean isTree){
		if (plugin.getConfig().getBoolean("config.greenhouse.ENABLED")){
			...
			double nc = Math.random();
			double rate = plugin.getConfig().getDouble("config.greenhouse.RATE");
			if (nc<rate){
				return isUnder;
			} else {
				return false;
